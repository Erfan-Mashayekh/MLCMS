...
class DiffusionMap:
    ...
    def compute_eigenfunctions(self, positions, sample, radius):
        """
        ...
        al = np.flip(t_hat_evals[-(sample+1):], axis=-1)
        vl = np.flip(t_hat_evecs[:,-(sample+1):], axis=-1)
    ...
    def compute_eigenfunctions_datafold(self, positions, sample):
        """
