...
def file_df_to_count_df(df,
    ...
    for pid in pedestrian_ids:
        ...
        #initialize pedestrian's health states and starting counting the number of these three states.
        current_state = simtime_group[0][1]
        if current_state == ID_SUSCEPTIBLE:
            group_counts.loc[group_counts['simTime'] >= 0, 'group-s'] += 1 
        elif current_state == ID_REMOVED: 
            group_counts.loc[group_counts['simTime'] >= 0, 'group-r'] += 1  
        else: group_counts.loc[group_counts['simTime'] >= 0, 'group-i'] += 1
        for (st, g) in simtime_group:
            #update pedestrian's health states until it becomes removed. and count the number of these three states.
