...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	/**
	 * Initialises the "Group Identity" of a Pedestrian in the SIR Model
	 */
	...
	/**
	 * Updates the simulation state for the next time step
	 * @param simTimeInSec
	 */
	@Override
	public void update(final double simTimeInSec) {
		...
		double sideLength = (1.1 * attributesSIRG.getInfectionMaxDistance());
		LinkedCellsGrid<Pedestrian> linkedCellsGrid = new LinkedCellsGrid<>(new VRectangle(topography.getBounds()), sideLength);

		if (c.getElements().size() > 0) {
			// sort Pedestrians into cells
			...
			for(Pedestrian p : c.getElements()) {
				...
				if (g.getID() == SIRType.ID_INFECTED.ordinal()) {
					// recovery mechanism
					if (this.random.nextDouble() < attributesSIRG.getRecoveryRate() * simTimeInSec) {
						elementRemoved(p);
						assignToGroup(p, SIRType.ID_REMOVED.ordinal());
					}
					// if we are infected then we cant get infected
					continue;
				}
